...
(function julvoxP5StructuredExplainability01(){
  ...
  function renderExplanation(decision){var explanation=classifyDecisionExplanation(decision);return'<div class="jvp5ex-wrap"><p class="jvp5ex-intro">Explication structurée de l’instantané enregistré. Les éléments favorables et défavorables sont relatifs à l’achat. Julvox ne recalcule pas la décision et n’ajoute aucun motif absent des données persistées.</p><div class="jvp5ex-grid"><section class="jvp5ex-section"><h3>Pour l’achat</h3>'+renderItems(explanation.favorable,'Aucun élément favorable explicite à l’achat dans les règles persistées.')+'</section><section class="jvp5ex-section"><h3>Contre l’achat</h3>'+renderItems(explanation.unfavorable,'Aucun élément défavorable explicite à l’achat dans les règles persistées.')+'</section><section class="jvp5ex-section"><h3>Incertitudes</h3>'+renderUncertainties(explanation.uncertainties)+'</section><section class="jvp5ex-section"><h3>Ce qui ferait réévaluer la décision</h3>'+renderConditions(explanation.changeConditions)+'</section></div></div>'}
...
function verify(input) {
  ...
  if (!html.includes('Pour l’achat') || !html.includes('Contre l’achat')) fail('purchase-relative favorable and unfavorable labels are required');
