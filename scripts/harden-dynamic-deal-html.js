...
function replaceRequired(text, search, replacement, label) {
  ...
  if (count !== 1) throw new Error(`P6.28 expected exactly one ${label}, found ${count}`);
...
function transformBlock(text, startMarker, endMarker, label, transform) {
  ...
  if (start < 0 || secondStart >= 0) throw new Error(`P6.28 expected exactly one ${label} start marker`);
...
(function julvoxDynamicDealHtmlTrust(){
  ...
  function openReportFromDeal(event, id) {
    ...
    if (deal && typeof window.openReport === 'function') window.openReport(positiveId(deal.id), text(deal.name, 300));
  }
...
function hardenHtml(input) {
  ...
  if (!html.includes('julvox-frontend-reconciliation-01-finalize-runtime')) throw new Error('P6.28 requires the reconciled public artifact first');
  ...
  html = transformBlock(html, 'function sanitizeRealDeals(deals) {', 'async function _refreshDealsBackground()', 'deal ingress normalizer', () => SAFE_SANITIZE_DEALS);
  ...
  html = transformBlock(
    ...
    'deal-of-day loader',
    block => replaceRequired(block, '    const deal = (data.deals || [])[0];', '    const deal = window.JulvoxDynamicDealTrust.normalizeDeal((data.deals || [])[0], false);', 'deal-of-day normalization'),
  );

  html = transformBlock(html, 'function renderDealOfDay(deal, el) {', 'let favorites = new Set();', 'deal-of-day renderer', block => {
    let out = block;
    out = replaceRequired(
      out,
    ...
    out = replaceRequired(out, '${(deal.image_url && deal.image_url.startsWith(\'http\'))', '${safeImage', 'deal-of-day raw image predicate');
    out = replaceRequired(out, '<img src="${deal.image_url}" alt="${deal.name}"', '<img src="${trust.html(safeImage)}" alt="${safeName}"', 'deal-of-day raw image attributes');
    ...
    out = replaceRequired(out, 'onclick="openDeal(${JSON.stringify(deal).replace(/"/g,\'&quot;\')})"', 'onclick="openDeal(${safeDealId})"', 'deal-of-day serialized object handler');
    return out;
  });
  ...
  html = transformBlock(html, 'function renderFlash() {', 'function startCountdowns() {', 'legacy flash renderer', () => `function renderFlash() {
...
`);
  ...
  html = transformBlock(html, 'function renderFlashLive(flashDeals) {', 'function startCountdownsLive(flashDeals) {', 'live flash renderer', block => {
    let out = block;
    ...
    out = replaceRequired(out, "      var url = this.getAttribute('data-url');\n      if (url && url.startsWith('http')) {\n        window.open(url, '_blank', 'noopener,noreferrer');", "      var url = window.JulvoxDynamicDealTrust.httpUrl(this.getAttribute('data-url'));\n      if (url) {\n        window.open(url, '_blank', 'noopener,noreferrer');", 'live flash click URL verification');
    return out;
  });

  html = transformBlock(html, 'async function openDeal(id) {', 'async function _enrichDealModal(deal) {', 'deal modal renderer', block => {
    let out = block;
    ...
    out = replaceRequired(out, 'var u=buildSmartUrl(deal); return u && u!==\'#\' ? `<a class="cta-main" href="${u}" target="_blank" rel="noopener" onclick="showToast(\'✅ Ouverture de ${escHtml(deal.store)}...\')">', 'var u=window.JulvoxDynamicDealTrust.httpUrl(buildSmartUrl(deal)); return u ? `<a class="cta-main" href="${window.JulvoxDynamicDealTrust.html(u)}" target="_blank" rel="noopener" onclick="showToast(\'✅ Ouverture en cours...\')">', 'modal CTA URL and handler');
    out = replaceRequired(out, '<button class="cta-sec" onclick="createAlert(\'${escHtml(deal.name)}\',${deal.current_price})">', '<button class="cta-sec" onclick="window.JulvoxDynamicDealTrust.createAlertFromDeal(${window.JulvoxDynamicDealTrust.positiveId(deal.id)})">', 'modal create-alert handler');
    return out;
  });

  html = transformBlock(html, 'async function _enrichDealModal(deal) {', 'function closeModal(e) {', 'deal analysis renderer', block => {
    let out = block;
    out = replaceRequired(out, ".map(r => '<div style=\"font-size:11px;color:var(--txt2);margin-bottom:3px\">→ ' + r + '</div>')", ".map(r => '<div style=\"font-size:11px;color:var(--txt2);margin-bottom:3px\">→ ' + window.JulvoxDynamicDealTrust.html(r) + '</div>')", 'analysis reason escaping');
    out = replaceRequired(out, "+ (rarity.label || '') + '</div>';", "+ window.JulvoxDynamicDealTrust.html(rarity.label || '') + '</div>';", 'analysis rarity escaping');
    out = replaceRequired(out, "+ '<div style=\"font-size:12px;font-weight:700;margin-bottom:4px\">' + (a.title || '') + '</div>'", "+ '<div style=\"font-size:12px;font-weight:700;margin-bottom:4px\">' + window.JulvoxDynamicDealTrust.html(a.title || '') + '</div>'", 'analysis alert title escaping');
    out = replaceRequired(out, "+ '<div style=\"font-size:11px;color:var(--txt2)\">' + (a.message || '') + '</div></div>'", "+ '<div style=\"font-size:11px;color:var(--txt2)\">' + window.JulvoxDynamicDealTrust.html(a.message || '') + '</div></div>'", 'analysis alert message escaping');
    return out;
  });
  ...
  for (const marker of forbidden) if (html.includes(marker)) throw new Error(`P6.28 unsafe dynamic deal sink remains: ${marker}`);
...
module.exports = { MARKER, RUNTIME, assertHardened, escapeHtml, hardenHtml, hardenPublicArtifact, normalizeHttpUrl };
