...
for (const fragment of expected) {
  const occurrences = distHtml.split(fragment).length - 1;
  if (occurrences !== 1) {
    errors.push(`Built index.html must contain exactly one expected brand metadata fragment: ${fragment}`);
  }
}

for (const legacyValue of legacyValues) {
  if (!sourceHtml.includes(legacyValue)) {
    errors.push(`Source index.html must preserve the historical value used by the migration: ${legacyValue}`);
  }
  if (distHtml.includes(legacyValue)) {
    errors.push(`Built index.html still contains historical brand metadata: ${legacyValue}`);
  }
}

const markerOccurrences = distHtml.split('runtime-contract:application.description').length - 1;
