...
function normalizeScoreFallbacks(source) {
  ...
  output = output.replace(
    /\(\s*(deal|d)\.novadeal_score\s*(?:\|\||\?\?)\s*\1\.score\s*\)\s*(?:\|\||\?\?)\s*(?:50|75|82)\b/g,
    (_, ref) => `ui00ResolveScore(${ref}.novadeal_score, ${ref}.score)`,
  );
  output = output.replace(
    /\b(deal|d)\.novadeal_score\s*(?:\|\||\?\?)\s*\1\.score\s*(?:\|\||\?\?)\s*(?:50|75|82)\b/g,
    (_, ref) => `ui00ResolveScore(${ref}.novadeal_score, ${ref}.score)`,
  );
  output = output.replace(
    /\b(deal|d)\.score\s*(?:\|\||\?\?)\s*\1\.novadeal_score\s*(?:\|\||\?\?)\s*(?:50|75|82)\b/g,
    (_, ref) => `ui00ResolveScore(${ref}.novadeal_score, ${ref}.score)`,
  );
