(function initProductionTruth(globalObject, factory) {
  ...
  const SUCCESS_KINDS = Object.freeze(['success', 'empty']);
  ...
  function retryButtonHtml() {
    return '<button type="button" data-ui00-retry style="margin-top:12px;padding:8px 12px;border-radius:8px;border:1px solid var(--border);background:var(--bg3);color:var(--txt);cursor:pointer">Réessayer</button>';
  }

  function renderState(element, kind, message, retry) {
    if (!element) return;
    const button = typeof retry === 'function' ? retryButtonHtml() : '';
  ...
  function renderPreservedError(element, message, retry) {
    if (!element) return;
    element.querySelector?.('[data-ui00-preserved-error]')?.remove?.();
    const documentLike = element.ownerDocument || globalObject?.document;
    if (!documentLike?.createElement) return;
    const notice = documentLike.createElement('div');
    notice.dataset.ui00PreservedError = 'true';
    notice.setAttribute('role', 'status');
    notice.style.cssText = 'margin:10px 0;padding:10px 12px;border:1px solid var(--border);border-radius:10px;color:var(--txt2);text-align:center';
    notice.innerHTML = `<div>${escapeHtml(message)}</div>${typeof retry === 'function' ? retryButtonHtml() : ''}`;
  ...
  function isConfirmedServerResult(result) {
    return result?.ok === true
      && SUCCESS_KINDS.includes(result.kind)
  ...
  const hasIdentifier = value => (Number.isInteger(value) && value > 0)
    || (typeof value === 'string' && value.trim().length > 0);
  const hasStatus = (data, allowed) => typeof data?.status === 'string' && allowed.includes(data.status);
  const confirmDeletion = (data, response) => response?.status === 204
    || data?.deleted === true
    || hasStatus(data, ['deleted', 'removed']);
  ...
  function installMutationOverrides() {
    ...
    globalObject.createAlert = async function(name, price) {
      ...
      }, { token, confirm: data => hasIdentifier(data?.alert_id) });
    ...
    globalObject.voteDeal = async function(id, vote) {
      ...
      const result = await client.post(`/deals/${id}/vote`, { vote }, {
        token,
        confirm: data => Number.isFinite(data?.up)
          && Number.isFinite(data?.down)
          && ['up', 'down', null].includes(data?.my_vote ?? null),
      });
    ...
    globalObject.submitCommunityDealNew = async function() {
      ...
      }, { token: currentToken(), confirm: data => hasIdentifier(data?.submission_id) });
    ...
    globalObject.voteCommDeal = async function(id, type, button) {
      ...
      const result = await client.post(`/community/deals/${id}/vote`, {
        ...
        token: currentToken(),
        confirm: data => Number.isFinite(data?.votes_validate) && Number.isFinite(data?.votes_reject ?? 0),
      });
    ...
    async function publishComment(path, input, reload) {
      ...
      const result = await client.post(path, { content: text }, {
        token: currentToken(), confirm: data => hasIdentifier(data?.comment_id),
      });
    ...
    globalObject.submitReport = async function() {
      ...
      const result = await client.post(`/deals/${globalObject.reportDealId}/report`, {
        ...
        token: currentToken(),
        confirm: data => hasIdentifier(data?.report_id) || hasStatus(data, ['reported', 'received', 'accepted']),
      });
    ...
    globalObject.createSquad = async function() {
      ...
      }, { token, confirm: data => hasIdentifier(data?.squad_id) });
    ...
    globalObject.addToWishlist = async function() {
      ...
      }, { token, confirm: data => hasIdentifier(data?.item?.id) });
    ...
    globalObject.removeFromWishlist = async function(id) {
      const result = await client.delete(`/wishlist/${id}`, {
        token: currentToken(),
        confirm: confirmDeletion,
      });
    ...
    globalObject.subscribeNewsletter = async function() {
      ...
      const result = await client.post('/newsletter/subscribe', {
        ...
        confirm: data => hasIdentifier(data?.subscriber_id)
          || hasStatus(data, ['subscribed', 'pending_confirmation', 'already_subscribed']),
      });
    ...
    globalObject.enableNotifPermission = async function() {
      ...
      if (!keyResult.ok || typeof keyResult.data?.public_key !== 'string' || !keyResult.data.public_key) {
        toast('❌ Clé de notification indisponible. Aucun abonnement créé.'); return;
      }
      let subscription;
      try {
    ...
    globalObject.votePromo = async function(id, type, button) {
      ...
      const result = await client.post(`/promos/${id}/vote`, { vote_type: type }, {
        token: currentToken(),
        confirm: data => (Number.isFinite(data?.votes_ok) && Number.isFinite(data?.votes_ko ?? 0))
          || hasStatus(data, ['voted', 'reported']),
      });
  ...
  return Object.freeze({
    ...
    renderState,
    renderPreservedError,
    isConfirmedServerResult,
